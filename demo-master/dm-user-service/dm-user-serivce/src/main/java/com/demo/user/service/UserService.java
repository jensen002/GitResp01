...
import com.demo.user.utils.CodecUtils;
...
public class UserService {
    ...
    public void sendVerifyCode(String phone) {
        ...
        map.put("phone", phone);
        map.put("code", code);
        amqpTemplate.convertAndSend("dm.sms.exchange", "sms.verify.code", map);
    }
    ...
    public void register(User user, String code) {
        ...
        if (!StringUtils.equals(code, value)) {
            //验证码不匹配
            throw new DmException(ExceptionEnum.VERIFY_CODE_NOT_MATCHING);
        }
    ...
    public User queryUser(String username, String password) {
        ...
        if (user == null) {
            throw new DmException(ExceptionEnum.USER_NOT_EXIST);
        }

        //检验密码是否正确
        if (!StringUtils.equals(CodecUtils.md5Hex(password, user.getSalt()), user.getPassword())) {
            //密码不正确
            throw new DmException(ExceptionEnum.PASSWORD_NOT_MATCHING);
        }
