...
import com.demo.common.utils.IdWorker;
import com.demo.common.utils.JsonUtils;
import com.demo.common.vo.PageResult;
import com.demo.item.pojo.Sku;
import com.demo.order.client.AddressClient;
import com.demo.order.client.GoodsClient;
import com.demo.order.dto.AddressDTO;
import com.demo.order.dto.OrderDto;
import com.demo.order.dto.OrderStatusEnum;
import com.demo.order.filter.LoginInterceptor;
import com.demo.order.mapper.OrderDetailMapper;
import com.demo.order.mapper.OrderMapper;
import com.demo.order.mapper.OrderStatusMapper;
import com.demo.order.pojo.Order;
import com.demo.order.pojo.OrderDetail;
import com.demo.order.pojo.OrderStatus;
import com.demo.order.utils.PayHelper;
...
public class OrderService {
    ...
    public Long createOrder(OrderDto orderDto) {
        ...
        if (addressDTO == null) {
            // 商品不存在，抛出异常
            throw new DmException(ExceptionEnum.RECEIVER_ADDRESS_NOT_FOUND);
        }
        ...
        if (CollectionUtils.isEmpty(skus)) {
            throw new DmException(ExceptionEnum.GOODS_NOT_FOUND);
        }
        ...
        try {
            ...
            amqpTemplate.convertAndSend("dm.cart.exchange", "cart.delete", JsonUtils.toString(map));
        } catch (Exception e) {
    ...
    public String generateUrl(Long orderId) {
        ...
        if (order.getOrderStatus().getStatus() != OrderStatusEnum.INIT.value()) {
            throw new DmException(ExceptionEnum.ORDER_STATUS_EXCEPTION);
        }

        //todo 这里传入一份钱，用于测试使用，实际中使用订单中的实付金额
        String url = payHelper.createPayUrl(orderId, "乐优商城测试", /*order.getActualPay()*/1L);
        if (StringUtils.isBlank(url)) {
            throw new DmException(ExceptionEnum.CREATE_PAY_URL_ERROR);
        }
    ...
    public Order queryById(Long orderId) {
        ...
        if (order == null) {
            throw new DmException(ExceptionEnum.ORDER_NOT_FOUND);
        }
        OrderDetail orderDetail = new OrderDetail();
        orderDetail.setOrderId(orderId);
